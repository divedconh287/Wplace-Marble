...
const metaContent = fs.readFileSync('src/conhMarble.meta.js', 'utf8');
...
esbuild.build({
  ...
  bundle: true,
  outfile: 'dist/conhMarble.user.css',
  minify: true
});
...
const resultEsbuild = await esbuild.build({
  ...
  outfile: 'dist/conhMarble.user.js', // The file the bundled code is exported to
...
fs.writeFileSync('dist/conhMarble.user.js', resultTerser.code, 'utf8');
...
if (!isGitHub) {
  try {
    importedMapCSS = JSON.parse(fs.readFileSync('dist/conhMarble.user.css.map.json', 'utf8'));
  } catch {
...
const mapCSS = mangleSelectors({
  inputPrefix: 'bm-',
  outputPrefix: 'bm-',
  pathJS: 'dist/conhMarble.user.js',
  pathCSS: 'dist/conhMarble.user.css',
...
if (mapCSS) {
  fs.writeFileSync('dist/conhMarble.user.css.map.json', JSON.stringify(mapCSS, null, 2));
}

// Adds the banner
fs.writeFileSync(
  'dist/conhMarble.user.js', 
  metaContent + fs.readFileSync('dist/conhMarble.user.js', 'utf8'), 
  'utf8'
);
