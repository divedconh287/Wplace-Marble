...
const metaContent = fs.readFileSync('src/Marble.meta.js', 'utf8');
...
const resultEsbuild = await esbuild.build({
  ...
  outfile: 'dist/Marble.user.js', // The file the bundled code is exported to
...
fs.writeFileSync('dist/Marble.user.js', resultTerser.code, 'utf8');
...
if (!isGitHub) {
  try {
    importedMapCSS = JSON.parse(fs.readFileSync('dist/Marble.user.css.map.json', 'utf8'));
  } catch {
...
const mapCSS = mangleSelectors({
  inputPrefix: 'bm-',
  outputPrefix: 'bm-',
  pathJS: 'dist/Marble.user.js',
  pathCSS: 'dist/Marble.user.css',
...
if (mapCSS) {
  fs.writeFileSync('dist/Marble.user.css.map.json', JSON.stringify(mapCSS, null, 2));
}

// Adds the banner
fs.writeFileSync(
  'dist/Marble.user.js', 
  metaContent + fs.readFileSync('dist/Marble.user.js', 'utf8'), 
  'utf8'
);
